...
def read_error_data(root: str):
    error_path = []  # 存储所有ssim误差图的路径
    supported = [".jpg", ".JPG", ".png", ".PNG", ".tif", ".TIF"]  # 支持的文件后缀类型
    assert os.path.exists(root), "dataset root: {} does not exist.".format(root)
    error = [os.path.join(root, i) for i in os.listdir(root) if os.path.splitext(i)[-1] in supported]
    for err_path in error:
        error_path.append(err_path)
    return error_path
...
def read_data(root: str):
    ...
    # 获取该类别对应的索引
    image_class = int(class_dicts['fake_stargan'])
    # print(class_dicts['celeba_pggan'])
    #image_class = int(class_dicts['img_realign_celeba_png'])
    # print(class_dicts['img_realign_celeba_png'])
    # 记录该类别的样本数量
...
def clean_save(images,fileName,i):
    ...
    for j in range(images.size(0)):
        image = images[j, :, :, :]
        utils.save_image(image, fileName + str(i)+"f_" + str(j) + ".jpg")
...
def adv_save(images, fileName, i):
    ...
    for j in range(images.size(0)):
        image = images[j, :, :, :]
        utils.save_image(image, fileName + str(i) + "a_" + str(j) + ".jpg")
...
def perturbation_save(perturbation,filename,i):
    ...
    for j in range(perturbation.size(0)):
        ...
        cv2.imwrite(filename + str(i) + "p_" + str(j) + ".jpg", image * 255)
