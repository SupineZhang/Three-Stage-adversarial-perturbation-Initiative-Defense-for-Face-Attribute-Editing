...
class Generator(nn.Module):
    def __init__(self, enc_dim=64, enc_layers=5, enc_norm_fn='batchnorm', enc_acti_fn='lrelu',
                 ...
                 c_dim=13, shortcut_layers=1, inject_layers=0, img_size=128):
        ...
        for i in range(dec_layers):
            if i < dec_layers - 1:
                ...
                n_in = n_in + c_dim if self.inject_layers > i else n_in
            else:
                layers += [ConvTranspose2dBlock(
...
class Discriminator(nn.Module):
    ...
    def __init__(self, dim=64, norm_fn='instancenorm', acti_fn='lrelu',
                 fc_dim=1024, fc_norm_fn='none', fc_acti_fn='lrelu',  c_dim=5, n_layers=5, img_size=128):
        ...
        self.fc_cls = nn.Sequential(
            LinearBlock(1024 * self.f_size * self.f_size, fc_dim, fc_norm_fn, fc_acti_fn),
            LinearBlock(fc_dim, c_dim, 'none', 'none')
        )
...
class Classifier(nn.Module):
    def __init__(self, enc_dim=4, fc_dim=1024,  c_dim=5, fc_norm_fn='none', fc_acti_fn='lrelu'):
        super(Classifier, self).__init__()
        self.fc_cls = nn.Sequential(
            LinearBlock( 1024*enc_dim*enc_dim, fc_dim, fc_norm_fn, fc_acti_fn),
            LinearBlock(fc_dim, c_dim, 'none', 'none')
        )

    def forward(self, z):
        h = z.contiguous().view(z.size(0), -1)
...
class AttGAN():
    ...
    def trainG(self, img_a, att_a, att_a_, att_b, att_b_,att_diff,att_diff_):
        ...
        img_fake = self.G(zs_a, att_diff_, mode='dec')
    ...
    def trainD(self, img_a, att_a, att_a_, att_b, att_b_,att_diff,att_diff_):
        for p in self.D.parameters():
            p.requires_grad = True
        
        img_fake = self.G(img_a, att_diff_).detach()
